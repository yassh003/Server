...
def tableData():
    ...
    for a in range(1,6):
        for g in range(1,3):
            ...
            # overall visitors 
            sqlO = text('SELECT COUNT(id) AS overall_visitors FROM visitors_log WHERE date = "'+currentDate+'" AND gender = "'+str(g)+'" AND age_group = "'+str(a)+'"')
            resultO = db.engine.execute(sqlO)
            rawdataO = resultO.fetchall()
            overallVisitors = rawdataO[0].overall_visitors
...
def bargraphdata():
    ...
    for m in range(1, 13):
        ...
        sql = text('SELECT COUNT(id) AS month_visitors FROM visitors_log WHERE MONTH(date) = "'+str(m)+'"')
        result = db.engine.execute(sql)
        rawdata = result.fetchall()
